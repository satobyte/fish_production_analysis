...
# =====================
# Preprocess Cage 2 (stocking + harvest + transfers, excl. stocking transfer)
# =====================
...
def preprocess_cage2(feeding, harvest, sampling, transfers=None):
    ...
    stocking_row = pd.DataFrame([{
        "DATE": stocking_date, "CAGE NUMBER": cage_number, "AVERAGE BODY WEIGHT(G)": initial_abw_g
    }])

    # Base timeline (sampling + stocking)
    ...
    # Harvest cumulatives (fish & kg) → align to sampling rows
    ...
    if transfers is not None and not transfers.empty:
        t = transfers.copy()
        t = t[t["DATE"] > stocking_date]  # exclude stocking transfer
    ...
    # Standing fish (for standing biomass)
