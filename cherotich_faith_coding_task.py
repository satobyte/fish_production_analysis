# app.py — Fish Cage Production Analysis (Biomass, ABW, eFCR) + Transfers
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Uppercase headers, trim, collapse spaces; normalize common variants."""
    df = df.copy()
...
def load_data(feeding_file, harvest_file, sampling_file, transfer_file=None):
    ...
    if transfers is not None:
        ...
        # standardize transfer weight column if needed
...
def preprocess_cage2(feeding, harvest, sampling, transfers=None):
    ...
    # Initialize cumulative columns to avoid KeyErrors
    for col in ["HARV_CUM_FISH", "IN_FISH_CUM", "OUT_FISH_CUM", "IN_KG_CUM", "OUT_KG_CUM"]:
        base[col] = 0.0

    # Harvest cumulative (fish)
    ...
    # Transfers cumulative (fish & kg), aligned to sampling dates
    if transfers is not None and not transfers.empty:
        ...
        if not t_out.empty:
            if num_col_t:
                ...
                merged = pd.merge_asof(base[["DATE"]], t_out.sort_values("DATE")[["DATE","OUT_FISH_CUM"]],
                                       on="DATE", direction="backward")
                base["OUT_FISH_CUM"] = merged["OUT_FISH_CUM"].fillna(0)
            if w_col_t:
                t_out["OUT_KG_CUM"] = pd.to_numeric(t_out[w_col_t], errors="coerce").fillna(0).cumsum()
                merged = pd.merge_asof(base[["DATE"]], t_out.sort_values("DATE")[["DATE","OUT_KG_CUM"]],
                                       on="DATE", direction="backward")
                base["OUT_KG_CUM"] = merged["OUT_KG_CUM"].fillna(0)
        ...
        if not t_in.empty:
            if num_col_t:
                ...
                merged = pd.merge_asof(base[["DATE"]], t_in.sort_values("DATE")[["DATE","IN_FISH_CUM"]],
                                       on="DATE", direction="backward")
                base["IN_FISH_CUM"] = merged["IN_FISH_CUM"].fillna(0)
            if w_col_t:
                t_in["IN_KG_CUM"] = pd.to_numeric(t_in[w_col_t], errors="coerce").fillna(0).cumsum()
                merged = pd.merge_asof(base[["DATE"]], t_in.sort_values("DATE")[["DATE","IN_KG_CUM"]],
                                       on="DATE", direction="backward")
                base["IN_KG_CUM"] = merged["IN_KG_CUM"].fillna(0)
...
# =====================
# Compute summary (Biomass, ABW, eFCR, transfers per period)
# =====================
...
def compute_summary(feeding_c2, sampling_c2):
    ...
    sampling_c2["TOTAL_WEIGHT_KG"] = (
        pd.to_numeric(sampling_c2["FISH_ALIVE"], errors="coerce").fillna(0) * sampling_c2["ABW_G"].fillna(0) / 1000.0
    )
    ...
    # tidy selection
    keep = [
        "DATE","CAGE NUMBER","NUMBER OF FISH","ABW_G","TOTAL_WEIGHT_KG",
        "CUM_FEED","AGGREGATED_eFCR","PERIOD_eFCR",
        "PERIOD_TRANSFER_IN_KG","PERIOD_TRANSFER_OUT_KG","NET_TRANSFER_KG",
        "PERIOD_TRANSFER_IN_FISH","PERIOD_TRANSFER_OUT_FISH","NET_TRANSFER_FISH",
    ]
...
if feeding_file and harvest_file and sampling_file:
    ...
    # Quick mock cages (no transfer simulation there)
    ...
    show_cols = [
        "DATE","NUMBER OF FISH","ABW_G","TOTAL_WEIGHT_KG",
    ...
    if selected_kpi == "Biomass":
        if "TOTAL_WEIGHT_KG" in df.columns:
            fig = px.line(df.dropna(subset=["TOTAL_WEIGHT_KG"]),
                          x="DATE", y="TOTAL_WEIGHT_KG", markers=True,
        ...
        if "ABW_G" in df.columns:
            fig = px.line(df.dropna(subset=["ABW_G"]),
