...
if feeding_file and harvest_file and sampling_file:
    ...
    if selected_kpi == "Growth":
        fig = px.line(df, x='DATE', y='TOTAL_WEIGHT_KG', markers=True, title=f"Cage {selected_cage} Growth Over Time")
        st.plotly_chart(fig)
    else:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df['DATE'], y=df['AGGREGATED_eFCR'], mode='lines+markers', name='Aggregated eFCR'))
        fig.add_trace(go.Scatter(x=df['DATE'], y=df['PERIOD_eFCR'], mode='lines+markers', name='Period eFCR'))
        fig.update_layout(title=f"Cage {selected_cage} eFCR Over Time", yaxis_title='eFCR')
        st.plotly_chart(fig)
else:
    st.info("Please upload Feeding Record, Fish Harvest, and Fish Sampling Excel files.")
